...
class ExampleUnitTest {
    ...
    fun aes_helper_isCorrect() {
        ...
        Assert.assertEquals(key1.length, AESHelper.keyLength)

        key = "12345678"
        val key2=AESHelper.pkcs5Padding(key)
        Assert.assertEquals(key2.length, AESHelper.keyLength)

        key = "123456781234567812345678"
        val key3=AESHelper.pkcs5Padding(key)
        Assert.assertEquals(key3.length, AESHelper.keyLength)
        println("key1:$key1")
        println("key2:$key2")
        println("key3:$key3")
    }
